<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=280&color=0:0078D4,100:00C6FF&text=NISHANT%20LATA&fontSize=50&fontColor=ffffff&animation=fadeIn&desc=DevOps%20Lead%20|%20Azure%20DevOps%20Engineer%20|%2011+Years+Experience&descAlignY=60"/>

<img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&size=30&duration=3000&pause=1000&color=00C6FF&center=true&vCenter=true&width=900&lines=☁️+Azure+Cloud+Specialist;🚀+CI%2FCD+Pipeline+Architect;🐳+Docker+%7C+Kubernetes+Expert;🏗️+Terraform+Infrastructure+Automation;⚡+Building+Scalable+Cloud+Platforms"/>

<img src="https://raw.githubusercontent.com/onimur/.github/master/.resources/git-header.svg" width="550">

<h3>
☁️ DevOps Lead | Azure DevOps Engineer | Cloud Automation Specialist | Platform Engineering Enthusiast
</h3>

<p>
Building Enterprise CI/CD Platforms • Automating Infrastructure • Engineering Reliability
</p>

<p>
<a href="https://github.com/YOUR_GITHUB_USERNAME">
<img src="https://komarev.com/ghpvc/?username=YOUR_GITHUB_USERNAME&label=Profile%20Views&color=0078D4&style=for-the-badge"/>
</a>

<a href="https://github.com/YOUR_GITHUB_USERNAME?tab=followers">
<img src="https://img.shields.io/github/followers/YOUR_GITHUB_USERNAME?style=for-the-badge"/>
</a>

<img src="https://img.shields.io/badge/Experience-11%2B%20Years-success?style=for-the-badge"/>

</p>

</div>

---

# 🚀 About Me

- 💼 Working as **Implementation Manager / DevOps Lead**
- ☁️ Specialized in **Microsoft Azure & Azure DevOps**
- 🏗️ Designing and Implementing **Enterprise CI/CD Platforms**
- ⚡ Automating Infrastructure using **Terraform, ARM Templates & Ansible**
- 🐳 Managing Containerized Workloads using **Docker & Kubernetes (AKS)**
- 🔐 Passionate about **DevSecOps, Platform Engineering & Reliability**
- 🌱 Currently exploring **GitOps, SRE and AI-driven DevOps Automation**
- 💬 Ask me about **Azure, Azure DevOps, Terraform, Kubernetes, Linux and Cloud Infrastructure**

---

# 🛠️ Tech Stack

<p align="center">

<img src="https://skillicons.dev/icons?i=azure,docker,kubernetes,terraform,github,git,linux,bash,ansible,powershell,githubactions"/>

</p>

---

# ☁️ Cloud & DevOps Expertise

```yaml
Cloud:
  - Microsoft Azure
  - Azure Virtual Machines
  - Azure Networking
  - VNet
  - NSG
  - Load Balancer
  - Application Gateway
  - Azure Storage
  - Azure Key Vault
  - Azure Monitor
  - Azure Container Registry
  - Azure Kubernetes Service (AKS)

DevOps:
  - Azure DevOps
  - Azure Pipelines
  - YAML Pipelines
  - GitHub Actions
  - CI/CD Architecture
  - Release Management
  - Deployment Automation

Infrastructure as Code:
  - Terraform
  - ARM Templates
  - Infrastructure Provisioning
  - State Management
  - Modular Infrastructure Design

Containers:
  - Docker
  - Docker Compose
  - Kubernetes
  - AKS
  - Helm

Automation:
  - Ansible
  - PowerShell
  - Bash

Operating Systems:
  - Linux
  - Windows Server

Version Control:
  - Git
  - GitHub
  - Azure Repos

Monitoring:
  - Azure Monitor
  - Log Analytics
  - Application Insights
```

---

# 💼 Core Competencies

✅ Azure Cloud Architecture

✅ Azure DevOps Engineering

✅ Enterprise CI/CD Design

✅ Infrastructure Automation

✅ Kubernetes Administration

✅ Terraform Infrastructure Provisioning

✅ Cloud Migration & Modernization

✅ DevSecOps Implementation

✅ Monitoring & Observability

✅ High Availability & Reliability Engineering

---

# ⚡ What I Do

```text
☁️ Build Enterprise Azure Platforms
🚀 Design End-to-End CI/CD Pipelines
🏗️ Provision Infrastructure using Terraform
🐳 Deploy and Manage Kubernetes Workloads
🔐 Integrate Security into DevOps Lifecycle
📊 Implement Monitoring & Observability
⚡ Automate Infrastructure and Operations
📈 Improve Platform Reliability and Release Velocity
```

---

# 📊 GitHub Analytics

<p align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&theme=tokyonight"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&layout=compact&theme=tokyonight"/>

<img height="180em" src="https://streak-stats.demolab.com?user=YOUR_GITHUB_USERNAME&theme=tokyonight"/>

</p>

---

# 🏆 GitHub Trophies

<p align="center">

<img src="https://github-profile-trophy.vercel.app/?username=YOUR_GITHUB_USERNAME&theme=tokyonight&row=1&column=7"/>

</p>

---

# 🐍 Contribution Snake

<p align="center">

<img src="https://raw.githubusercontent.com/YOUR_GITHUB_USERNAME/YOUR_GITHUB_USERNAME/output/github-contribution-grid-snake-dark.svg"/>

</p>

---

# 🤝 Connect With Me

<p align="center">

<a href="https://www.linkedin.com/in/YOUR_LINKEDIN">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:YOUR_EMAIL">
<img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

</p>

---

<div align="center">

### ☁️ Automating Infrastructure • Accelerating Delivery • Engineering Reliability 🚀

</div>
````
